    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-2xl bg-gray-900/80 border border-gray-800 space-y-3">
        <label className="block text-xs font-semibold text-gray-300 flex items-center gap-2">
          <Regex className="w-4 h-4 text-indigo-400" /> Regular Expression Pattern
        </label>
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="e.g. [a-z]+"
              className="w-full glass-input pl-6 pr-3 py-2 rounded-xl text-sm font-mono text-indigo-300"
            />
            <span className="absolute right-3 top-2.5 text-gray-500 font-mono text-sm">
              /{Object.entries(flags).filter(([, v]) => v).map(([k]) => k).join('')}
            </span>
          </div>

          {/* Flags Toggles */}
          <div className="flex items-center gap-2 bg-gray-950 p-1.5 rounded-xl border border-gray-800 text-xs">
            {Object.keys(flags).map((flag) => (
              <label key={flag} className="flex items-center gap-1 px-2 py-1 rounded cursor-pointer select-none">
                <input
                  ...
                  className="w-3.5 h-3.5 rounded bg-gray-900 border-gray-700 text-indigo-500"
                />
                <span className="font-mono text-gray-300 uppercase">{flag}</span>
              </label>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <span className="text-gray-400 font-medium py-1">Presets:</span>
        {[
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-700"
          >
            {p.name}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          <label className="block text-xs font-semibold text-gray-300">Test Target Text</label>
          <textarea
            rows={12}
            value={testText}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="block text-xs font-semibold text-gray-300">Match Results</label>
            {error ? (
          ...
          <div className="p-4 rounded-2xl bg-gray-950 border border-gray-800 space-y-2 max-h-[350px] overflow-y-auto font-mono text-xs">
            {error ? (
              <div className="p-3 rounded-xl bg-red-500/10 text-red-400">{error}</div>
            ) : matches.length === 0 ? (
              <div className="text-gray-500 py-12 text-center">No matches found for current pattern</div>
            ) : (
              matches.map((m, idx) => (
                <div key={idx} className="p-3 rounded-xl bg-gray-900 border border-gray-800 space-y-1">
                  <div className="flex items-center justify-between text-gray-400 text-[11px]">
                    <span className="text-indigo-400 font-bold">Match #{idx + 1}</span>
                    <span>Index: {m.index}</span>
                  </div>
                  <div className="text-emerald-300 font-bold break-all bg-gray-950 p-2 rounded-lg border border-gray-800">
                    "{m.text}"
                  </div>
                  {m.groups.length > 0 && (
