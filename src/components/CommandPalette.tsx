  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4 bg-black/70 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-2xl bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        {/* Search Input Bar */}
        <div className="flex items-center px-4 py-3.5 border-b border-gray-800 bg-gray-900/80">
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all ${
                    isSelected ? 'bg-indigo-600/20 border border-indigo-500/30 text-white' : 'hover:bg-gray-800/60 text-gray-300'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${isSelected ? 'bg-indigo-500 text-white' : 'bg-gray-800 text-indigo-400'}`}>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {tool.badge && (
                          <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 font-bold">
                            {tool.badge}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-400">
                    <span className="hidden sm:inline text-gray-500">{tool.category}</span>
                    <ArrowRight className={`w-4 h-4 ${isSelected ? 'text-indigo-400' : 'text-gray-600'}`} />
                  </div>
                </div>
              );
            })
        ...
        {/* Footer shortcuts */}
        <div className="px-4 py-2 bg-gray-950/80 border-t border-gray-800 flex items-center justify-between text-xs text-gray-400">
          <div className="flex items-center gap-3">
            <span>
              <kbd className="px-1.5 py-0.5 bg-gray-800 rounded text-gray-300 border border-gray-700">↑</kbd>{' '}
              <kbd className="px-1.5 py-0.5 bg-gray-800 rounded text-gray-300 border border-gray-700">↓</kbd> navigate
            </span>
            <span>
              <kbd className="px-1.5 py-0.5 bg-gray-800 rounded text-gray-300 border border-gray-700">↵</kbd> select
            </span>
          </div>
          <span>
            <kbd className="px-1.5 py-0.5 bg-gray-800 rounded text-gray-300 border border-gray-700">esc</kbd> close
          </span>
        </div>
      </div>
    </div>
